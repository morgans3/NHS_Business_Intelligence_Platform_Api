...
module.exports.getAllActioned = function (limit, roles, callback) {
    ...
    if (rolecheck === "" || rolecheck === "error") {
        ...
        const query =
            ...
            " WHERE " +
            rolecheck +
            " \"status\" NOT LIKE 'Pending' ORDER BY updated_date DESC LIMIT $1;";
        pool.query(query, [limit], (error, results) => {
            if (error) {
                console.log("Error: " + error);
                callback(error, null, null);
...
module.exports.updateStatus = function (id, status, nonReferralReason, callback) {
    let reason = "";
    if (nonReferralReason) {
    ...
    pool.query(geoquery, [status, id, nonReferralReason], callback);
};
