...
using(nhs_number)`;
const nonestatement = `M.asthma IS NOT TRUE AND M.chd IS NOT TRUE AND M.heart_failure IS NOT TRUE
AND M.cancer IS NOT TRUE AND M.copd IS NOT TRUE AND
...
M.rheumatoid_arthritis IS NOT TRUE AND M.stroke_tia IS NOT TRUE AND M.palliative_care_flag IS NOT TRUE
 AND M.psychotic_disorder_flag IS NOT TRUE AND
M.spl IS NOT TRUE AND M.chemo_radiotherapy IS NOT TRUE AND M.haematological_cancers IS NOT TRUE AND
 M.rare_diseases IS NOT TRUE AND M.respiratory IS NOT TRUE`;
...
module.exports.getHistoryByNHSNumber = function (nhsnumber, roles, callback) {
    ...
    if (rolecheck === "" || rolecheck === "error") {
        ...
        const query =
            selectjoin.replace("public.population_master", "public.population_history") +
            " WHERE " +
            rolecheck +
            ` "nhs_number" = '` +
            nhsnumber +
