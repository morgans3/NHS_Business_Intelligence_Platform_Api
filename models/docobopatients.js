...
module.exports.addPatient = function (orgcode, patientId, resource, callback) {
    ...
    const values = [
        ...
        nhs,
        patientId,
        resource.dischargeDestination,
        resource.offBoardingDate,
        resource.onBoardingDate,
        resource.referralSource,
        resource.selfDischarge,
        resource.serviceType,
        resource.patientDied,
    ];
    const geoquery =
        "INSERT INTO public." +
        tablename +
        ` ("orgcode", "nhsNumber", "patientId", "dischargeDestination", "offBoardingDate",
        "onBoardingDate", "referralSource", "selfDischarge", "serviceType", "patientDied")
...
function updateO2Stats(patientId, resource, callback) {
    ...
    pool.query(delquery, (err, result) => {
        if (err) {
            ...
            const gberror = "";
            const geoquery =
                "INSERT INTO public." + satstablename + `("patientId", "measurmentDateTime", "o2Sat") VALUES ($1, $2, $3) RETURNING *`;
            async.mapSeries(
                resource,
                (values, outerCB) => {
                ...
                function (errAsyncMap, results) {
                    if (errAsyncMap) {
                        callback(errAsyncMap, gberror);
                    } else {
                        callback(null, true);
                    }
                }
